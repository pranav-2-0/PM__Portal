...
exports.discrepancyReportService = exports.DiscrepancyReportService = void 0;
...
class DiscrepancyReportService {
    ...
    `);
        ...
        const healthScore = totalEmp > 0
            ...
            : 100;
        const insertResult = await database_1.default.query(`INSERT INTO pm_discrepancy_snapshots
         (triggered_by, total_employees, total_pms, health_score, summary)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [triggeredBy, totalEmp, parseInt(row.total_pms) || 0, healthScore, JSON.stringify(summary)]);
        logger_1.logger.info('PM discrepancy snapshot generated', { triggeredBy, healthScore, summary });
        return this.mapRow(insertResult.rows[0]);
    }
    /** Fetch the most recent snapshot */
    async getLatestSnapshot() {
        await this.ensureTable();
        const result = await database_1.default.query(`SELECT * FROM pm_discrepancy_snapshots ORDER BY created_at DESC LIMIT 1`);
    ...
    async getHistory() {
        await this.ensureTable();
        const result = await database_1.default.query(`SELECT id, triggered_by, total_employees, total_pms, health_score, summary, created_at
     ...
     */
    async getDetails(type, page = 1, pageSize = 50) {
        ...
        const QUERIES = {
          ...
          LIMIT $1 OFFSET $2`,
            ...
            pm_overloaded: {
                count: `SELECT COUNT(*) FROM people_managers WHERE is_active = true AND reportee_count >= max_capacity`,
                data: `
          SELECT pm.employee_id, pm.name, pm.email,
                 pm.practice, pm.sub_practice, pm.grade,
                 pm.reportee_count, pm.max_capacity,
                 ROUND((pm.reportee_count::numeric / NULLIF(pm.max_capacity, 0)) * 100, 1) AS utilization_pct
          FROM people_managers pm
          WHERE pm.is_active = true AND pm.reportee_count >= pm.max_capacity
        ...
        const [countResult, dataResult] = await Promise.all([
            database_1.default.query(QUERIES[type].count),
            database_1.default.query(QUERIES[type].data, [pageSize, offset]),
        ]);
        return {
...
exports.DiscrepancyReportService = DiscrepancyReportService;
exports.discrepancyReportService = new DiscrepancyReportService();
