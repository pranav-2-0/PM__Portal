...
const getGradeLevelLocal = (grade) => GRADE_ORDER.indexOf((grade || '').toUpperCase().trim());
...
// ─── Flag builder ───────────────────────────────────────────────────────────────
const mkFlag = (severity, code, message) => ({ severity, code, message });
function classifySkill(pmSkill, empSkill, clusterMap) {
    if (!pmSkill || !empSkill)
        return { tier: 'E4', score: 0 };
    const pm = pmSkill.toLowerCase().trim();
    const emp = empSkill.toLowerCase().trim();
    if (pm === emp)
        return { tier: 'E1', score: 1.0 };
    ...
    const sim = (0, gradeUtils_1.calculateSkillSimilarity)(pmSkill, empSkill);
    if (sim >= 0.5)
        return { tier: 'E2', score: sim };
    return { tier: 'E4', score: 0 };
}
// ─── Matching Service ───────────────────────────────────────────────────────────
class MatchingService {
    ...
    async getCandidatePMs(practice) {
        const longLeavePattern = MatchingService.LONG_LEAVE_TYPES
            ...
            .join(' OR ');
        const result = await database_1.default.query(`SELECT pm.*,
    ...
    async persistException(employeeId, type, description) {
        try {
            await database_1.default.query(`INSERT INTO exceptions (employee_id, exception_type, description, status)
         VALUES ($1, $2, $3, 'open')
         ON CONFLICT DO NOTHING`, [employeeId, type, description]);
        }
        catch { /* non-blocking */ }
    }
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    async getCandidatePMsRelaxed(practice) {
        const longLeavePattern = MatchingService.LONG_LEAVE_TYPES
            .map(t => t.replace(/'/g, "''"))
            .map(t => `LOWER(pm.leave_type) LIKE '%${t}%'`)
            .join(' OR ');
       ...
       ORDER BY
         ...
         pm.reportee_count ASC`, [practice]);
        return result.rows;
    }
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    scorePMRelaxed(pm, employee, weights, clusterMap) {
        const flags = [];
        const reasons = ['[FORCED] PM_RESIGNATION_OVERRIDE — normal path failed'];
        const warnings = ['Forced assignment: isPMResigned=true, all constraints relaxed'];
        let score = 0;
        ...
        if (samePractice) {
            ...
            flags.push(mkFlag('Critical', 'CROSS_PRACTICE_FORCED', `Relaxation: PM practice "${pm.practice}" differs from employee practice "${employee.practice}". Forced by resignation override.`));
        ...
        if (pm.region === employee.region) {
            ...
            flags.push(mkFlag('Major', 'REGION_RELAXED', `Relaxation 3: Region constraint ignored — employee: ${employee.region}, PM: ${pm.region}`));
        }
        // BU
        if (pm.cu === employee.cu) {
            ...
            flags.push(mkFlag('Major', 'CROSS_BU_RELAXED', `Relaxation 2: Cross-BU — employee: ${employee.cu}, PM: ${pm.cu}`));
        }
        ...
        if (skillResult.tier === 'E1') {
            ...
            flags.push(mkFlag('Major', 'SKILL_WEAK_RELAXED', `Relaxation 4: Weak skill match accepted — PM: "${pm.skill}" vs employee: "${employee.skill}"`));
        }
        else {
            // E4 — no skill match, but FORCED so we include with score=0 for skill
            flags.push(mkFlag('Critical', 'SKILL_NONE_RELAXED', `Relaxation 4: No skill match — PM: "${pm.skill ?? 'N/A'}" vs employee: "${employee.skill ?? 'N/A'}". Assigned anyway by resignation override.`));
        ...
        if (pmLevel > empLevel) {
            ...
            flags.push(mkFlag('Major', 'GRADE_RELAXED', `Relaxation 6: Level constraint relaxed — PM grade ${pm.grade} is not above employee grade ${employee.grade}`));
        }
        ...
        if ((pm.reportee_count || 0) >= 10) {
            flags.push(mkFlag('Major', 'CAPACITY_RELAXED', `Relaxation 5: Capacity constraint relaxed — PM has ${pm.reportee_count}/${cap} reportees (over normal cap of 10)`));
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    scorePM(pm, employee, weights, clusterMap, isCrossBU // true only when evaluated as a cross-BU fallback
    ) {
        const flags = [];
        const reasons = [];
        const warnings = [];
        ...
        if (sameRegion) {
            ...
            flags.push(mkFlag('Major', 'NO_PM_SAME_REGION', `B2: No PM in same region — employee: ${employee.region}, PM: ${pm.region}. Falling back to BU check.`));
        ...
        if (isCrossBU) {
            ...
            flags.push(mkFlag('Major', 'NO_PM_SAME_BU', `C2→D3: No PM found in employee BU "${employee.cu}". Cross-BU PM "${pm.cu}" used. Lowest confidence path.`));
        ...
        if (sameBU && sameAccount) {
            ...
            if (employee.account && pm.account) {
                flags.push(mkFlag('Minor', 'SAME_BU_DIFF_ACCOUNT', `D2: Same BU but different account — employee: ${employee.account}, PM: ${pm.account}.`));
                warnings.push(`Different account: ${employee.account} vs ${pm.account}`);
            }
            if (!isCrossBU)
                matchTier = 'same_bu_diff_account';
            if (confidence === 'High')
                confidence = 'Medium';
        }
        ...
        if (skillResult.tier === 'E4') {
            ...
            if (!pm.skill || !employee.skill) {
                flags.push(mkFlag('Critical', 'SKILL_DATA_MISSING', `E4: No skill data for ${!pm.skill ? 'PM' : 'employee'}. Cannot evaluate skill alignment. Manual review required.`));
            }
            else {
                flags.push(mkFlag('Critical', 'SKILL_NO_MATCH', `E4: No acceptable skill proximity — PM: "${pm.skill}" vs employee: "${employee.skill}". Manual review required.`));
            }
            return null;
        }
        if (skillResult.tier === 'E1') {
        ...
        if (skillResult.tier === 'E2') {
            ...
            flags.push(mkFlag('Major', 'SKILL_CLUSTER_MATCH', `E2: Exact skill not found; closest acceptable match used — PM: "${pm.skill}" vs employee: "${employee.skill}".`));
            warnings.push(`Closest skill used: ${pm.skill} ≈ ${employee.skill}`);
            if (confidence === 'High')
                confidence = 'Medium';
            // E3: closest skill in a cross-BU context → further reduce confidence
            if (isCrossBU) {
                flags.push(mkFlag('Major', 'SKILL_CROSS_SCOPE', `E3: Closest skill used in a cross-BU context — confidence further reduced.`));
                confidence = 'Low';
                matchTier = 'cross_bu';
            }
        }
        ...
        if (utilPct >= 80)
            warnings.push(`High utilization: ${utilPct.toFixed(0)}% of available pool capacity`);
        // ── Spec path label ─────────────────────────────────────────────────────────
        let path;
        if (isCrossBU) {
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    async findBestPM(employee, isPMResigned = false) {
        ...
        if (allPMs.length === 0) {
            await this.persistException(employee.employee_id, 'no_pm_in_practice', `A2/Path8: No PMs in practice "${employee.practice}" passed pre-validation ` +
        ...
        let gradePMs = allPMs.filter(pm => (0, gradeUtils_1.isGradeEligible)(pm.grade, employee.grade) && (0, gradeUtils_1.isGradeGapAllowed)(pm.grade, employee.grade));
        ...
        if (gradePMs.length === 0) {
            logger_1.logger.warn('Grade escalation triggered — no PM at N+1..N+3', {
                employeeId: employee.employee_id,
                grade: employee.grade,
            });
            gradePMs = allPMs.filter(pm => (0, gradeUtils_1.isGradeEligible)(pm.grade, employee.grade));
        }
        if (gradePMs.length === 0) {
            await this.persistException(employee.employee_id, 'no_eligible_grade_pm', `No PMs in practice "${employee.practice}" at any eligible grade above ` +
        ...
        if (candidates.length === 0 && isPMResigned) {
            logger_1.logger.warn('PM_RESIGNATION_OVERRIDE activated — relaxing all constraints', {
                employeeId: employee.employee_id,
                practice: employee.practice,
                cu: employee.cu,
                region: employee.region,
            });
            await this.persistException(employee.employee_id, 'pm_resignation_override', `isPMResigned=TRUE: Normal matching returned 0 candidates. ` +
            ...
            if (relaxedPMs.length === 0) {
                ...
                await this.persistException(employee.employee_id, 'resignation_override_failed', `CRITICAL: isPMResigned=TRUE but NO active PMs exist anywhere. ` +
                    `Manual intervention required immediately.`);
                logger_1.logger.error('Resignation override failed — no PMs available globally', {
                    employeeId: employee.employee_id,
                });
                return [];
            }
            // Score all relaxed candidates (Relaxations 2+4+6 applied in scorePMRelaxed)
            const relaxedCandidates = relaxedPMs.map(pm => this.scorePMRelaxed(pm, employee, weights, clusterMap));
            ...
            logger_1.logger.info('Forced assignment selected via PM_RESIGNATION_OVERRIDE', {
                employeeId: employee.employee_id,
            ...
            await this.persistException(employee.employee_id, 'forced_assignment_made', `FORCED_ASSIGNMENT: PM ${forcedBest.pm.employee_id} assigned to employee ` +
        ...
        if (candidates.length === 0) {
            await this.persistException(employee.employee_id, 'no_valid_pm_found', `Path 9/10: No PM found for employee ${employee.employee_id}. ` +
        ...
        const sorted = candidates.sort((a, b) => {
            ...
            // F2: longest tenure
            const aTenure = a.pm.tenure_days ?? 0;
            const bTenure = b.pm.tenure_days ?? 0;
            if (bTenure !== aTenure) {
        ...
        if (best.confidence === 'Low') {
            await this.persistException(employee.employee_id, 'low_confidence_match', `Low confidence match (${best.matchTier}). PM: ${best.pm.employee_id}. ` +
        ...
        if (bestSkillScore < 0.7 && employee.skill) {
            logger_1.logger.warn('Poor skill match', {
                employeeId: employee.employee_id,
                skill: employee.skill,
                bestSkillScore,
            });
            await this.persistException(employee.employee_id, 'skill_mismatch', `No PM with strong skill match. Employee skill: ${employee.skill}. ` +
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    validateDatasetScope(employees) {
        const practices = [...new Set(employees.map(e => (e.practice || '').trim()).filter(Boolean))];
        if (practices.length <= 1)
            return { isScoped: true, practices };
        return {
            isScoped: false,
            practices,
            criticalFlag: mkFlag('Critical', 'DATASET_NOT_PRACTICE_SCOPED', `A2: Dataset is not practice-scoped — ${practices.length} distinct practices found: ` +
    ...
    async assignPM(employeeId, pmId, assignmentType, score) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO pm_assignments (employee_id, old_pm_id, new_pm_id, match_score, assignment_type, status)
...
MatchingService.LONG_LEAVE_TYPES = [
