...
const reassignmentService_1 = require("./reassignmentService");
...
class WorkflowAutomationService {
    constructor() {
        ...
        this.reassignmentService = new reassignmentService_1.ReassignmentService();
    }
    /**
    ...
    async processNewJoiners() {
        try {
            logger_1.logger.info('Starting new joiner assignment workflow');
      ...
      `);
            ...
            logger_1.logger.info(`Found ${newJoiners.length} new joiners to process`);
            for (const employee of newJoiners) {
               ...
               VALUES ($1, 'no_pm_found', 'No eligible PM found for new joiner', 'open')`, [employee.employee_id]);
                        logger_1.logger.warn('No PM match found', { employeeId: employee.employee_id });
                        continue;
                    }
                    // Get best match
                    const bestMatch = matches[0];
                    // Create assignment
                    const assignmentResult = await database_1.default.query(`INSERT INTO pm_assignments 
             (employee_id, old_pm_id, new_pm_id, assignment_type, match_score, status)
             VALUES ($1, $2, $3, 'new_joiner', $4, 'pending')
             RETURNING id`, [employee.employee_id, employee.current_pm_id, bestMatch.pm.employee_id, bestMatch.score]);
                    const assignmentId = assignmentResult.rows[0].id;
                    // Create approval workflow
                    await this.approvalService.createApprovalWorkflow(assignmentId, undefined, // No old PM for new joiners
                    bestMatch.pm.email);
                    // Send notifications
                    await this.notificationService.notifyPMAssignment(employee.name, bestMatch.pm.name, bestMatch.pm.email);
                    await this.notificationService.notifyApprovalRequired(assignmentId, 'dcx@capgemini.com', employee.name);
                    logger_1.logger.info('New joiner assignment created', {
                    ...
                    logger_1.logger.error('Error processing new joiner', { employeeId: employee.employee_id, error });
                }
            }
            return { processed: newJoiners.length };
        }
        catch (error) {
            logger_1.logger.error('Error in new joiner workflow', error);
            throw error;
        }
    }
    /**
    ...
    async processReassignments() {
        try {
            logger_1.logger.info('Starting PM reassignment workflow');
            ...
            logger_1.logger.info('PM reassignment workflow completed');
            return { success: true };
        }
        catch (error) {
            logger_1.logger.error('Error in reassignment workflow', error);
            throw error;
        }
    }
    /**
    ...
    async processMonthlyEngagement() {
        try {
            logger_1.logger.info('Starting monthly engagement workflow');
            // Get all active PMs
            const pmsResult = await database_1.default.query(`
        SELECT * FROM people_managers WHERE is_active = true
      `);
            for (const pm of pmsResult.rows) {
                // Get team snapshot
                const teamResult = await database_1.default.query(`
...
Current Capacity: ${pm.reportee_count}/10
...
PM Alignment System
        `;
                ...
                logger_1.logger.info('Monthly snapshot sent', { pmId: pm.employee_id });
            }
            return { pmsNotified: pmsResult.rows.length };
        }
        catch (error) {
            logger_1.logger.error('Error in monthly engagement workflow', error);
            throw error;
        }
    }
    /**
    ...
    `);
        for (const mismatch of result.rows) {
         ...
         ON CONFLICT (employee_id, exception_type) WHERE status = 'open' DO NOTHING`, [
            ...
            logger_1.logger.warn('Data mismatch detected', {
     ...
     */
    async checkBenchSeparationMismatch() {
        const result = await database_1.default.query(`
    ...
    async processPendingApprovals() {
      ...
      `);
            for (const approval of result.rows) {
                ...
                logger_1.logger.info('Approval reminder sent', {
            ...
            logger_1.logger.error('Error processing pending approvals', error);
            throw error;
        }
    }
}
