...
exports.getFileHeaders = exports.parseSkillReportExcel = exports.getSkillReportHeaders = exports.parseGADExcel = exports.extractPMsFromGAD = exports.parseSeparationExcel = exports.parsePMExcel = exports.parseEmployeeExcel = void 0;
 ...
 */
const findEmployeeDataSheet = (workbook) => {
    let best = null;
 ...
 */
const collectPMSheetRows = (workbook) => {
    const combined = [];
...
const parseEmployeeExcel = (buffer) => {
    try {
        ...
        const parsedData = data.map((row) => {
            ...
            const pmEmailForResolution = String(row['People Manager Email Address'] || row['Supervisor Email ID'] || '').trim();
            ...
            const employeeIdRaw = normalizeId(row['Employee ID'] || row['employee_id'] || ggid || pernrVal || '');
            ...
            let name = String(row['Full Name'] || row['full_name'] ||
                row['Name'] || row['name'] ||
                row['Employee Name'] || row['Resource Name'] ||
                row['Display Name'] ||
                `${row['First Name'] || ''} ${row['Middle Name'] || ''} ${row['Last Name'] || ''}`.trim() || '').trim();
            ...
            const practice = String(row['Practice'] || row['practice'] ||
            ...
            const grade = String(row['Unified Grade'] || row['Global Grade'] ||
                row['Grade'] || row['grade'] ||
                row['Local Grade'] || '').trim().toUpperCase();
            // ── SKILL ──
            const skill = String(row['Skill'] || row['skill'] ||
                row['Emp Skills'] ||
                row['Primary Skill'] || row['R2D2 - Primary Skill'] ||
                row['Skill Group'] || row['R2D2 - Skill Group'] || '').trim() || undefined;
            ...
            const joiningDateVal = row['Joining Date'] ||
                row['Global Date of Joining'] ||
                row['Most Recent Hire Date'] ||
                row['Local Date of Joining'];
            const joiningDateParsed = joiningDateVal ? new Date(joiningDateVal) : undefined;
            ...
            const subPractice = String(row['Sub Practice'] || row['SubPractice'] || row['Sub-practice'] || row['Sub BU Name'] || row['Sub BUName'] || '').trim() || undefined;
            const locationVal = String(row['Location'] || row['Location Standardization'] || row['Work Location'] || row['Office Location'] || '').trim() || undefined;
...
exports.parseEmployeeExcel = parseEmployeeExcel;
const parsePMExcel = (buffer) => {
    try {
        const workbook = XLSX.read(buffer, { type: 'buffer' });
        ...
        if (isBenchFormat) {
            ...
            for (const row of data) {
                ...
                const pmGgid = normalizeId(row['PM GGID'] || row['PM Global Id'] || row['PM Global ID'] || '');
                if (!pmGgid)
                    continue;
                if (pmMap.has(pmGgid))
                    continue; // already captured this PM
                ...
                let name = pmName;
                if (!name && pmEmail) {
                    name = pmEmail.split('@')[0].split('.').map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(' ');
                }
                if (!name)
                    name = `PM-${pmGgid}`;
                pmMap.set(pmGgid, {
            ...
            parsedData = data.map((row) => {
                ...
                if (!name && emailStr) {
                    name = emailStr.split('@')[0].split('.').map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(' ');
                }
                if (!name && employeeId)
                    name = `PM-${employeeId}`;
                let defaultCapacity = 10;
...
exports.parsePMExcel = parsePMExcel;
/**
...
const parseSeparationExcel = (buffer) => {
    try {
        ...
        const parsedData = data.map((row) => {
            ...
            // ── Name ──
            const personName = cleanValue(getColValue(row, ['Employee Name', 'Resource Name', 'Name', 'Full Name', 'Emp Name']) ?? '') || undefined;
            // ── Designation ──
            const designation = cleanValue(getColValue(row, ['Designation', 'Role', 'Title', 'Job Title', 'Position']) ?? '') || undefined;
            // ── Grade ──
            const grade = cleanValue(getColValue(row, ['Global Grade', 'Grade', 'Unified Grade', 'Local Grade', 'Band']) ?? '').toUpperCase() || undefined;
            // ── Separation type & reason ──
            const separationType = cleanValue(getColValue(row, ['Separation Type', 'Sep Type', 'SeparationType', 'Exit Type']) ?? '');
            const reasonText = cleanValue(getColValue(row, [
            ...
            const ok = !!sep.employee_id && !!sep.lwd && !isNaN(sep.lwd.getTime());
            return ok;
        });
...
exports.parseSeparationExcel = parseSeparationExcel;
/**
 ...
 */
const extractPMsFromGAD = (buffer) => {
    try {
        const workbook = XLSX.read(buffer, { type: 'buffer' });
        ...
        for (const row of data) {
            ...
            const pmId = normalizeId(row['People Manager ID'] || row['People Manager GGID'] ||
                row['PM GGID'] || row['PM Global Id'] ||
                row['PM Global ID'] || row['Supervisor GGID'] ||
                row['Supervisor ID'] || '');
            if (!pmId || pmMap.has(pmId))
                continue;
            const pmName = String(row['People Manager Name'] || row['People Manager Full Name'] ||
                row['PM Name'] || row['Supervisor Full Name'] || '').trim();
            const pmEmail = String(row['People Manager Email Address'] || row['People Manager Email'] ||
                row['PM Email ID'] || row['Supervisor Email ID'] || '').trim();
            const pmGrade = cleanValue(row['PM Grade'] || row['People Manager Grade'] || row['Supervisor Grade'] || '').toUpperCase() || 'C1';
            // PM practice: take from the employee row's practice (they share the same org)
            const pmPractice = String(row['PM Practice'] || row['PM Sub-practice'] ||
                row['Practice'] || row['Service Line'] ||
                row['BU Name'] || row['BUName'] || '').trim();
            const pmCu = String(row['CU'] || row['Organization BU'] || row['SBU Name'] || pmPractice || '').trim();
            const pmRegion = String(row['PM Region'] || row['Region'] || row['Location Standardization'] || '').trim();
            let defaultCapacity = 10;
            if (pmGrade.startsWith('E'))
                defaultCapacity = 25;
            else if (pmGrade.startsWith('D'))
                defaultCapacity = 15;
            let name = pmName;
            if (!name && pmEmail) {
                name = pmEmail.split('@')[0].split('.').map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(' ');
...
exports.extractPMsFromGAD = extractPMsFromGAD;
/**
...
const parseGADExcel = (buffer) => {
    try {
        ...
        const parsedData = data.map((row) => {
            ...
            // ── Name ──
            let name = String(row['Full Name'] || row['Employee Name'] || row['Resource Name'] ||
                `${row['First Name'] || ''} ${row['Last Name'] || ''}`.trim() || '').trim();
            if (!name && employeeId)
                name = `Employee-${employeeId}`;
            // ── Email ──
            let email = String(row['Email ID'] || row['Email'] || row['Email Address'] || row['Work Email'] ||
                row['Corporate Email'] || row['Official Email'] || row['NT Login ID'] || '').trim();
            if (!email && employeeId)
                email = `${employeeId.toLowerCase()}@capgemini.com`;
            // ── Practice & Sub-Practice ──
            const practice = String(row['Practice'] || row['Sub Practice'] || row['Sub-practice'] ||
                row['Sub BU Name'] || row['Sub BUName'] || row['Service Line'] || '').trim();
            const subPractice = String(row['Sub Practice'] || row['SubPractice'] || row['Sub Practice 2'] ||
                row['Sub-practice'] || row['Sub BU Name'] || '').trim() || undefined;
            // ── CU ──
            const cu = String(row['CU'] || row['Capability Unit'] || row['Organization BU'] ||
                row['SBU Name'] || row['Business Area'] || practice || '').trim();
            // ── Region / Location ──
            const region = String(row['Region'] || row['Location Standardization'] || row['Location'] || '').trim();
            const locationVal = String(row['Location'] || row['Work Location'] || row['Office Location'] ||
                row['City'] || row['Location Standardization'] || '').trim() || undefined;
            // ── Grade ──
            const grade = cleanValue(row['Unified Grade'] || row['Global Grade'] || row['Grade'] || row['grade'] || row['Local Grade'] || '').toUpperCase();
            // ── Skill (placeholder: extracted if present, else null — Skill Report upload fills it later) ──
            const skill = cleanValue(row['Primary Skill'] || row['R2D2 - Primary Skill'] || row['Skill'] || row['Emp Skills'] ||
                row['Skill Group'] || row['R2D2 - Skill Group'] || '') || undefined;
            // ── Account ──
            const account = cleanValue(row['Account'] || row['Ultimate Account Name'] || row['Account Name'] || '') || undefined;
            // ── Joining date: prefer CG Start Date for tenure calc ──
            const joiningDateVal = row['CG Start Date'] || row['Most Recent Hire Date'] ||
            ...
            const pmIdDirect = normalizeId(row['People Manager ID'] || row['People Manager GGID'] ||
                row['PM GGID'] || row['PM Global Id'] ||
                row['PM Global ID'] || row['Supervisor GGID'] ||
                row['Supervisor ID'] || '');
            const pmEmailForResolution = String(row['People Manager Email Address'] || row['PM Email ID'] ||
                row['Supervisor Email ID'] || '').trim();
            if (!employeeId) {
...
exports.parseGADExcel = parseGADExcel;
...
const getSkillReportHeaders = (buffer) => {
    try {
        const workbook = XLSX.read(buffer, { type: 'buffer' });
        const result = {};
        for (const sheetName of workbook.SheetNames) {
...
exports.getSkillReportHeaders = getSkillReportHeaders;
const parseSkillReportExcel = (buffer) => {
    try {
        ...
        if (!found) {
            ...
            for (const sn of workbook.SheetNames) {
                ...
                if (rows.length > 0)
                    console.log(`  Sheet "${sn}" row-1:`, (rows[0] || []).map((c) => String(c || '').trim()));
            }
            return [];
        }
        const rawRows = XLSX.utils.sheet_to_json(found.sheet, { header: 1 });
        ...
        const colIndex = (aliases) => {
            ...
            for (let i = 0; i < headerRow.length; i++) {
                if (normalised.has(normalizeHeader(headerRow[i])))
                    return i;
            }
            return -1;
        };
...
exports.parseSkillReportExcel = parseSkillReportExcel;
/**
