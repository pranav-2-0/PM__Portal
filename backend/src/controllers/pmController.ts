...
// };
// import { Request, Response } from 'express';
// import 'multer'; // augments Express.Request with req.file (from @types/multer)
// import { DataIngestionService } from '../services/dataIngestionService';
// import { BulkUploadService } from '../services/bulkUploadService';
// import { MatchingService } from '../services/matchingService';
// import { StatisticsService } from '../services/statisticsService';
...
//   try {
//     const user = (req as any).user;
//     const userResult = await pool.query('SELECT department FROM users WHERE id = $1', [user?.id]);
...
//     const esc = (v: any): string => v != null ? `"${String(v).replace(/"/g, '""')}"` : '';
...
// export const getSkillManagementCoverage = async (req: Request, res: Response) => {
//   try {
//     const [practiceCount, gradeCount, cuCount, regionCount, totalEmp] = await Promise.all([
//       pool.query(`SELECT COUNT(DISTINCT practice)::int AS count FROM employees WHERE status = 'active' AND practice IS NOT NULL AND TRIM(practice) != ''`),
//       pool.query(`SELECT COUNT(DISTINCT grade)::int AS count FROM employees WHERE status = 'active' AND grade IS NOT NULL AND TRIM(grade) != ''`),
//       pool.query(`SELECT COUNT(DISTINCT cu)::int AS count FROM employees WHERE status = 'active' AND cu IS NOT NULL AND TRIM(cu) != ''`),
//       pool.query(`SELECT COUNT(DISTINCT region)::int AS count FROM employees WHERE status = 'active' AND region IS NOT NULL AND TRIM(region) != ''`),
...
//     });
//   } catch (error: any) {
//     logger.error('Error fetching skill management coverage', error);
//     res.status(500).json({ error: error.message });
//   }
// };

// export const getEmployeeSkillDistribution = async (req: Request, res: Response) => {
//   try {
//     const { practice, cu, region, grade, status = 'active' } = req.query as Record<string, string>;

//     let whereClause = 'WHERE e.status = $1';
//     const params: any[] = [status];
//     let idx = 2;

//     if (practice && practice !== 'All') { whereClause += ` AND e.practice = $${idx}`; params.push(practice); idx++; }
//     if (cu      && cu      !== 'All') { whereClause += ` AND e.cu = $${idx}`;       params.push(cu);       idx++; }
//     if (region  && region  !== 'All') { whereClause += ` AND e.region = $${idx}`;   params.push(region);   idx++; }
//     if (grade)                         { whereClause += ` AND e.grade = $${idx}`;    params.push(grade);    idx++; }

//     const [dist, total, skills] = await Promise.all([
//       pool.query(
//         `SELECT COALESCE(NULLIF(TRIM(e.skill), ''), 'Not Set') AS skill,
...
//         params,
//       ),
//       pool.query(`SELECT COUNT(*)::int AS total FROM employees e ${whereClause}`, params),
//       pool.query(
//         `SELECT COUNT(DISTINCT NULLIF(TRIM(e.skill), ''))::int AS total FROM employees e ${whereClause}`,
//         params,
//       ),
//     ]);
...
//     });
//   } catch (error: any) {
//     logger.error('Error fetching employee skill distribution', error);
//     res.status(500).json({ error: error.message });
//   }
// };

// // export const updateSingleEmployeeSkill = async (req: Request, res: Response) => {
...
// //     try {
// //       await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS primary_skill VARCHAR(500)`);
// //       logger.info('primary_skill column verified');
// //     } catch (e: any) {
// //       logger.warn('Could not verify primary_skill column:', e.message);
// //     }

// //     // Perform the update — set BOTH skill and primary_skill so the UI reflects the change
// //     console.log(`[SKILL UPDATE] Updating skill for ${employeeId} to "${trimmedSkill}"`);
// //     const updateResult = await pool.query(
// //       `UPDATE employees 
// //        SET skill = $1, primary_skill = $1, updated_at = CURRENT_TIMESTAMP 
// //        WHERE employee_id = $2`,
// //       [trimmedSkill, employeeId]
// //     );
...
//       console.log(`[SKILL UPDATE] FAIL - Missing or invalid employeeId`);
...
//     const trimmedSkill = skill.trim();
//     console.log(`[SKILL UPDATE] Trimmed skill: "${trimmedSkill}"`);

//     // STEP 2: Ensure primary_skill column exists (CRITICAL)
//     console.log(`[SKILL UPDATE] Checking primary_skill column...`);
//     try {
...
//         console.log(`[SKILL UPDATE] Creating primary_skill column...`);
//         await pool.query(`ALTER TABLE employees ADD COLUMN primary_skill VARCHAR(500)`);
//         console.log(`[SKILL UPDATE] ✓ primary_skill column created successfully`);
//       } else {
//         console.log(`[SKILL UPDATE] ✓ primary_skill column already exists`);
//       }
//     } catch (colErr: any) {
//       logger.error('CRITICAL: Error with primary_skill column:', colErr.message);
//       return res.status(500).json({ error: `Database schema error: ${colErr.message}` });
//     }

//     // STEP 3: Check if employee exists
//     console.log(`[SKILL UPDATE] Checking if employee ${employeeId} exists...`);
//     const empCheckResult = await pool.query(
...
//     const currentEmp = empCheckResult.rows[0];
//     console.log(`[SKILL UPDATE] Current state - skill: "${currentEmp.skill}", primary_skill: "${currentEmp.primary_skill}"`);

//     // STEP 4: Build update query based on whether primary_skill is already set
//     let updateQuery = '';
//     let updateParams: any[] = [];

//     if (!currentEmp.primary_skill) {
//       // First time: back up the original skill before overwriting
//       console.log(`[SKILL UPDATE] Backing up current skill as primary_skill`);
//       updateQuery = `UPDATE employees 
//                     SET skill = $1, primary_skill = $2, updated_at = CURRENT_TIMESTAMP 
//                     WHERE employee_id = $3`;
//       updateParams = [trimmedSkill, currentEmp.skill || trimmedSkill, employeeId];
//     } else {
//       // Already backed up: just update skill, keep primary_skill
//       console.log(`[SKILL UPDATE] Using existing primary_skill backup`);
//       updateQuery = `UPDATE employees 
//                     SET skill = $1, updated_at = CURRENT_TIMESTAMP 
//                     WHERE employee_id = $2`;
//       updateParams = [trimmedSkill, employeeId];
//     }

//     // STEP 5: Execute update
//     console.log(`[SKILL UPDATE] Executing update query...`);
//     const updateResult = await pool.query(updateQuery, updateParams);

//     if (updateResult.rowCount === 0) {
//       console.log(`[SKILL UPDATE] FAIL - Update affected 0 rows`);
//       return res.status(500).json({ error: 'Update failed: no rows affected' });
//     }

//     console.log(`[SKILL UPDATE] ✓ Update affected ${updateResult.rowCount} rows`);

//     // STEP 6: Verify the update succeeded
//     console.log(`[SKILL UPDATE] Verifying update...`);
...
//       console.log(`[SKILL UPDATE] FAIL - Employee disappeared after update`);
//       return res.status(500).json({ error: 'Verification failed: employee not found' });
//     }

//     const updatedEmp = verifyResult.rows[0];
//     if (updatedEmp.skill !== trimmedSkill) {
//       console.log(`[SKILL UPDATE] FAIL - Skill mismatch after update. Got: "${updatedEmp.skill}", Expected: "${trimmedSkill}"`);
//       return res.status(500).json({ error: 'Verification failed: skill not updated' });
//     }

//     const duration = Date.now() - startTime;
//     console.log(`[SKILL UPDATE] ✓✓✓ SUCCESS in ${duration}ms`);
//     console.log(`[SKILL UPDATE] Final state - skill: "${updatedEmp.skill}", primary_skill: "${updatedEmp.primary_skill}"`);

//     return res.json({ 
//       message: 'Skill updated successfully',
//       employeeId,
//       skill: trimmedSkill,
//       primary_skill: updatedEmp.primary_skill,
...
//     const errorMsg = error?.message || String(error) || 'Unknown error';
//     const duration = Date.now() - startTime;
//     console.error(`[SKILL UPDATE] ✗✗✗ EXCEPTION after ${duration}ms`);
//     console.error(`[SKILL UPDATE] Error Message:`, errorMsg);
//     console.error(`[SKILL UPDATE] Error Code:`, error?.code);
//     console.error(`[SKILL UPDATE] Error Detail:`, error?.detail);
//     logger.error('Skill update exception:', error);
...
//     const trimmedSkill = skill.trim();
//     console.log(`[BULK SKILL UPDATE] Starting bulk update with skill: "${trimmedSkill}"`);

//     // Ensure primary_skill column exists
//     try {
//       const columnCheckResult = await pool.query(
//         `SELECT column_name FROM information_schema.columns 
//          WHERE table_name='employees' AND column_name='primary_skill'`
...
//       if (columnCheckResult.rows.length === 0) {
//         console.log(`[BULK SKILL UPDATE] Creating primary_skill column...`);
//         await pool.query(`ALTER TABLE employees ADD COLUMN primary_skill VARCHAR(500)`);
//       }
      
//       // Backfill primary_skill from skill where primary_skill is NULL
//       const backfillResult = await pool.query(
//         `UPDATE employees SET primary_skill = skill
//          WHERE primary_skill IS NULL AND skill IS NOT NULL`
//       );
//       console.log(`[BULK SKILL UPDATE] Backfilled primary_skill for ${backfillResult.rowCount} employees`);
//     } catch (colErr: any) {
//       logger.error('Error with primary_skill column:', colErr.message);
//       // Don't fail the main update for column issues
//     }

//     let whereClause = `WHERE status = 'active'`;
//     const params: any[] = [trimmedSkill];
//     let idx = 2;
...
//     console.log(`[BULK SKILL UPDATE] Executing update with ${params.length - 1} filters`);

//     // Update employees with the new skill
//     const updateResult = await pool.query(
//       `UPDATE employees SET skill = $1, updated_at = CURRENT_TIMESTAMP ${whereClause}`,
//       params,
//     );

//     console.log(`[BULK SKILL UPDATE] ✓ Updated ${updateResult.rowCount} employees`);
//     logger.info(`Bulk skill update: "${trimmedSkill}" → ${updateResult.rowCount} employees`);
    
//     res.json({
//       message: `Successfully updated ${updateResult.rowCount} employee(s) with skill: ${trimmedSkill}`,
//       updatedCount: updateResult.rowCount || 0,
//       skill: trimmedSkill,
//       practice: practice || null,
//     });
//   } catch (error: any) {
//     console.error(`[BULK SKILL UPDATE] ERROR:`, error.message);
//     logger.error('Error bulk updating employee skills:', error);
...
//     logger.info(`Remove skill update: reverted ${result.rowCount} employees to primary skill`);
//     res.json({
//       message: 'Skill reverted to primary skill.',
//       updatedCount: result.rowCount,
//     });
//   } catch (error: any) {
//     logger.error('Error removing employee skill update', error);
//     res.status(500).json({ error: error.message });
//   }
// };
...
//     const pmResult = await pool.query(
//       `SELECT pm.employee_id, pm.name, pm.email, pm.practice, pm.cu, pm.region,
 ...
 */
const generateDiscrepancySnapshot = async (triggeredBy: string) => {
  // Run snapshot generation and MV refresh concurrently — both are independent.
  // Awaiting both ensures the No PM Found materialized view is fully up-to-date
  // before the upload response is returned to the client (no more stale data).
  const [snapResult] = await Promise.allSettled([
    discrepancyReportService.generateSnapshot(triggeredBy).catch((err: any) => {
      logger.warn('Discrepancy snapshot generation failed (non-blocking)', err);
      return undefined;
    }),
    // Await the alignment cache refresh so No PM Found tab is consistent immediately
    refreshAlignmentCache().catch((err: any) => {
      logger.warn('Alignment cache refresh failed after upload', err);
    }),
  ]);
  return snapResult.status === 'fulfilled' ? (snapResult.value as any)?.summary : undefined;
};
...
export const uploadBenchReport = async (req: Request, res: Response) => {
  try {
    ...
    // Leave/Bench uploads are intentionally NOT practice-scoped.
    // Process the full dataset and match records by global employee_id (GGID).
    const employees = allEmployees;

    logger.info(`Bench: ${allEmployees.length} total rows, ${employees.length} processed (practice-agnostic)`);
...
export const getNewJoinersList = async (req: Request, res: Response) => {
  try {
    const { practice: queryPractice, department_id, cu, region, page = '1', pageSize = '50' } = req.query;
    const pageNum = parseInt(page as string);
    const pageSizeNum = parseInt(pageSize as string);
    const offset = (pageNum - 1) * pageSizeNum;
...
export const getAllEmployees = async (req: Request, res: Response) => {
  try {
    ...
    FROM employees e LEFT JOIN people_managers pm ON e.current_pm_id = pm.employee_id WHERE 1=1`;
    let countQuery = 'SELECT COUNT(*) FROM employees e WHERE 1=1';
...
export const getAllSeparations = async (req: Request, res: Response) => {
  try {
    const { status, grade, skill, practice: queryPractice, department_id, cu, region, person_type, page = '1', pageSize = '50' } = req.query;
    const pageNum = parseInt(page as string);
    const pageSizeNum = parseInt(pageSize as string);
    const offset = (pageNum - 1) * pageSizeNum;
...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    const isPracticeScoped = filterPractice && user?.role !== 'Super Admin';
    ...
    const stats = await statsService.getDashboardStats(hasExplicitFilter ? filterPractice : undefined);
    res.json(stats);
  } catch (error: any) {
    logger.error('Error fetching dashboard stats', error);
    res.status(500).json({ error: error.message });
...
export const getPMCapacityReport = async (req: Request, res: Response) => {
  try {
    ...
    const report = await statsService.getPMCapacityReport(hasExplicitFilter ? filterPractice : undefined);
    res.json(report);
  } catch (error: any) {
    logger.error('Error fetching PM capacity report', error);
    res.status(500).json({ error: error.message });
...
export const getPracticeFilters = async (req: Request, res: Response) => {
  try {
    const { practice: queryPractice, department_id } = req.query as Record<string, string>;
    const user = (req as any).user;
    const userPractice = (user?.department_name || user?.practice || '').trim();
    const isSuperAdmin = user?.role === 'Super Admin';

    // Non-super-admin users are always scoped to their own practice.
    // Super Admin may optionally scope by department_id or explicit practice query.
    ...
    if (isSuperAdmin) {
      if (department_id) {
        ...
        effectivePractice = queryPractice;
      } else {
        effectivePractice = '';
      }
    }
    ...
    if (effectivePractice) {
      const [cusResult, regionsResult] = await Promise.all([
        pool.query(
          `SELECT DISTINCT cu FROM (
             SELECT cu FROM people_managers WHERE cu IS NOT NULL AND TRIM(cu) != '' AND practice = $1
             UNION
             SELECT cu FROM employees WHERE cu IS NOT NULL AND TRIM(cu) != '' AND practice = $1
           ) x
           ORDER BY cu`,
          [effectivePractice],
        ),
        pool.query(
          `SELECT DISTINCT region FROM (
             SELECT region FROM people_managers WHERE region IS NOT NULL AND TRIM(region) != '' AND practice = $1
             UNION
             SELECT region FROM employees WHERE region IS NOT NULL AND TRIM(region) != '' AND practice = $1
           ) x
           ORDER BY region`,
          [effectivePractice],
        ),
      ...
      const cus = cusResult.rows.map(r => r.cu);
      const regions = regionsResult.rows.map(r => r.region);

      return res.json({
        practices: ['All', effectivePractice],
        cus: ['All', ...cus],
        regions: ['All', ...regions],
      });
    }
...
export const getMisalignments = async (req: Request, res: Response) => {
  try {
    const { page = '1', pageSize = '50', type, practice, department_id } = req.query as any;
    const pageNum = parseInt(page as string);
    const pageSizeNum = parseInt(pageSize as string);
    const offset = (pageNum - 1) * pageSizeNum;
    ...
    const user = (req as any).user;
    const isSuperAdmin = user?.role === 'Super Admin';
    const userPractice = (user?.department_name || '').trim();

    let effectivePractice: string | null = null;
    if (isSuperAdmin) {
      if (department_id) {
        const deptResult = await pool.query('SELECT name FROM departments WHERE id = $1', [department_id]);
        effectivePractice = deptResult.rows[0]?.name || null;
      } else if (practice && practice !== 'All') {
        effectivePractice = String(practice);
      }
    } else {
      effectivePractice = userPractice || null;
    }

    const queryParams: any[] = [];
    let paramIndex = 1;

    const practiceClause = effectivePractice ? `AND e.practice = $${paramIndex++}` : '';
    if (effectivePractice) {
      queryParams.push(effectivePractice);
    }
    const typeFilter = type && type !== 'all' ? `AND mismatch_type = $${paramIndex++}` : '';
    if (type && type !== 'all') {
    ...
    const unmappedResult = effectivePractice
      ? await pool.query(
          `SELECT COUNT(*) FROM employees WHERE status = 'active' AND current_pm_id IS NULL AND practice = $1`,
          [effectivePractice]
        )
      : await pool.query(
          `SELECT COUNT(*) FROM employees WHERE status = 'active' AND current_pm_id IS NULL`
...
export const exportMisalignmentsCSV = async (req: Request, res: Response) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  try {
    const { type, practice, department_id } = req.query as any;
    const user = (req as any).user;
    const isSuperAdmin = user?.role === 'Super Admin';
    const userPractice = (user?.department_name || '').trim();
    ...
    const typeWhere = type && type !== 'all' ? `AND mismatch_type = $${paramIndex++}` : '';
    if (type && type !== 'all') {
      queryParams.push(type);
    }
...
export const getUnmappedEmployees = async (req: Request, res: Response) => {
  try {
    const { page = '1', pageSize = '50', format, practice, department_id } = req.query as any;
    const pageNum = parseInt(page as string);
    const pageSizeNum = parseInt(pageSize as string);
    const offset = (pageNum - 1) * pageSizeNum;
    ...
    const baseWhere = `
      ...
      ${effectivePractice ? 'AND e.practice = $1' : ''}
      AND NOT EXISTS (SELECT 1 FROM people_managers pm WHERE pm.employee_id = e.employee_id AND pm.is_active = true)
    `;
    const baseParams: any[] = effectivePractice ? [effectivePractice] : [];
    ...
    const countResult = await pool.query(`SELECT COUNT(*) ${baseWhere}`, baseParams);
    const totalCount = parseInt(countResult.rows[0].count);
...
export const getGADAnalysisSummary = async (req: Request, res: Response) => {
  try {
    const { practice, pm_id, department_id } = req.query as { practice?: string; pm_id?: string; department_id?: string };

    const user         = (req as any).user;
    const isSuperAdmin = user?.role === 'Super Admin';
    const userPractice = (user?.department_name || '').trim();

    // Resolve effective practice — same scoping logic used by all other GAD endpoints
    let effectivePractice: string | null = null;
    if (isSuperAdmin) {
      if (department_id) {
        const deptResult = await pool.query('SELECT name FROM departments WHERE id = $1', [department_id]);
      ...
      // SuperAdmin with no filter → show all practices (effectivePractice stays null)
    } else {
      // Non-super-admin is always scoped to their own practice
      effectivePractice = userPractice || null;
    }

    const pmId = pm_id && pm_id !== 'All' ? pm_id : null;

    const pf       = effectivePractice;
    const empPF    = pf   ? `AND e.practice = $1`                                  : '';
    const pmClause = pmId ? `AND e.current_pm_id = '${pmId.replace(/'/g, "''")}'` : '';
    const params   = pf   ? [pf]                                                    : [];
...
export const getSkillManagementCoverage = async (req: Request, res: Response) => {
  try {
    ...
    if (effectivePractice) {
      params.push(effectivePractice);
      where.push(`practice = $${params.length}`);
    }
    const whereClause = `WHERE ${where.join(' AND ')}`;

    const [practiceCount, gradeCount, cuCount, regionCount, totalEmp] = await Promise.all([
      pool.query(`SELECT COUNT(DISTINCT practice)::int AS count FROM employees ${whereClause} AND practice IS NOT NULL AND TRIM(practice) != ''`, params),
      pool.query(`SELECT COUNT(DISTINCT grade)::int AS count FROM employees ${whereClause} AND grade IS NOT NULL AND TRIM(grade) != ''`, params),
      pool.query(`SELECT COUNT(DISTINCT cu)::int AS count FROM employees ${whereClause} AND cu IS NOT NULL AND TRIM(cu) != ''`, params),
      pool.query(`SELECT COUNT(DISTINCT region)::int AS count FROM employees ${whereClause} AND region IS NOT NULL AND TRIM(region) != ''`, params),
      pool.query(`SELECT COUNT(*)::int AS count FROM employees ${whereClause}`, params),
    ]);
...
export const bulkUpdateEmployeeSkills = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      await pool.query(
        `UPDATE employees SET primary_skill = skill
         WHERE primary_skill IS NULL AND skill IS NOT NULL AND TRIM(skill) != ''`,
      );
    } catch (colErr: any) {
    ...
    logger.info(`Bulk skill update: "${skill.trim()}" → ${result.rowCount} employees`);
    res.json({
...
export const updateSingleEmployeeSkill = async (req: Request, res: Response) => {
  try {
    ...
    // Ensure primary_skill column exists and backfill once
    try {
      await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS primary_skill VARCHAR(500)`);
      await pool.query(
        `UPDATE employees SET primary_skill = skill
    ...
    logger.info(`Single skill update: employee ${employeeId} → "${skill.trim()}"`);
    res.json({ message: 'Skill updated.' });
  } catch (error: any) {
    logger.error('Error updating single employee skill', error);
    res.status(500).json({ error: error.message });
  }
};
...
// ─────────────────────────────────────────────────────────────────────────────
// No Suggested PM — paginated list
// Data source: mv_no_suggested_pm (materialized view, refreshed on every upload)
// ─────────────────────────────────────────────────────────────────────────────
export const getNoSuggestedPMEmployees = async (req: Request, res: Response) => {
  try {
    const { page = '1', pageSize = '50', department_id } = req.query as any;
    const pageNum     = parseInt(page     as string);
    const pageSizeNum = parseInt(pageSize as string);
    const offset      = (pageNum - 1) * pageSizeNum;
    ...
    let whereClause = '';
    if (effectivePractice) {
      whereClause = 'WHERE emp_practice = $1';
      params.push(effectivePractice);
    }

    const countResult = await pool.query(
      `SELECT COUNT(*) FROM mv_no_suggested_pm ${whereClause}`,
      params,
    );
    const totalCount = parseInt(countResult.rows[0].count);
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /employees/:employeeId/suggested-pms
// Returns top-5 best-matching PMs for a given employee (manual assign popup).
// ─────────────────────────────────────────────────────────────────────────────
export const getSuggestedPMsForEmployee = async (req: Request, res: Response) => {
  try {
    const { employeeId } = req.params;
    ...
    const longLeaveTypes = [
      ...
      'medical leave', 'medical-leave',
      'leave of absence', 'loa', 'extended leave',
    ];
    const longLeavePattern = longLeaveTypes
      .map(t => `LOWER(pm2.leave_type) LIKE '%${t.replace(/'/g, "''")}%'`)
      .join(' OR ');
