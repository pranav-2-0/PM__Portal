...
export class DataIngestionService {
  async bulkInsertEmployees(employees: Employee[]) {
    ...
    try {
      ...
      for (const emp of employees) {
        await client.query(
          `INSERT INTO employees (employee_id, name, email, practice, cu, region, account, skill, grade, current_pm_id, joining_date, is_new_joiner, sub_practice, location, hire_reason, bench_status, upload_source, leave_type, leave_start_date, leave_end_date)
      ...
      if (needsResolution.length > 0) {
        ...
        const pmEmails = needsResolution.map(e => e._pm_email as string);
        await client.query(
          `UPDATE employees e
  ...
  async bulkInsertPMs(pms: PeopleManager[]) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      for (const pm of pms) {
        ...
        await client.query(
          `INSERT INTO people_managers (employee_id, name, email, practice, cu, region, account, skill, grade, reportee_count, max_capacity, is_active, sub_practice, location, upload_source)
  ...
  async autoGeneratePMs(dryRun: boolean = true) {
    const query = `
      SELECT
        e.employee_id,
        e.name,
    ...
    const newOnes  = rows.filter((r: any) => !r.already_pm);
    const existing = rows.filter((r: any) => r.already_pm);
    ...
    try {
      ...
      for (const emp of rows) {
        const maxCapacity = getMaxCapacityForGrade(emp.grade);
        const result = await client.query(
          ...
          [emp.employee_id, emp.name, emp.email, emp.practice || '',
   ...
   */
  async autoAssignEmployees(dryRun: boolean = true) {
    // Fetch all unassigned employees who are not PMs themselves
    const { rows: unassigned } = await pool.query<Employee>(`
      SELECT e.*
      FROM employees e
    ...
    try {
      ...
      for (const emp of unassigned) {
        try {
          ...
          await client.query(
            `UPDATE employees SET current_pm_id = $1, updated_at = CURRENT_TIMESTAMP
             WHERE employee_id = $2`,
            [best.pm.employee_id, emp.employee_id]
          );
          ...
          await client.query(
            `INSERT INTO pm_assignments
             ...
             VALUES ($1, $2, $3, $4, 'new_joiner', 'completed', CURRENT_DATE)`,
            [emp.employee_id, emp.current_pm_id ?? null, best.pm.employee_id, best.score]
          );
          await client.query(
  ...
  async bulkInsertSeparations(separations: SeparationReport[]) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      ...
      const pmSet  = new Set(pmRes.rows.map((r: any) => r.employee_id));
      const empSet = new Set(empRes.rows.map((r: any) => r.employee_id));
        ...
        await client.query(
          `INSERT INTO separation_reports
           ...
           ON CONFLICT (employee_id) DO UPDATE SET
             ...
             updated_at      = CURRENT_TIMESTAMP`,
          [sep.employee_id, sep.lwd, sep.reason ?? null, sep.separation_type ?? null,
  ...
  async bulkInsertSkills(skills: SkillRepositoryEntry[]) {
    if (skills.length === 0) return { success: true, count: 0, inserted: 0 };

    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      const seen = new Map<string, SkillRepositoryEntry>();
      ...
      for (let i = 0; i < unique.length; i += BATCH) {
        ...
        const res = await client.query(
          `INSERT INTO skill_repository (practice, skill_name, skill_cluster)
