...
export default function Dashboard() {
  ...
  const utilizationData = [
    { name: 'Under 50%', value: capacityReport?.filter((pm: any) => (pm.reportee_count / pm.max_capacity) < 0.5).length || 0 },
    { name: '50-75%', value: capacityReport?.filter((pm: any) => {
      const util = pm.reportee_count / pm.max_capacity;
    ...
    { name: 'Over 90%', value: capacityReport?.filter((pm: any) => (pm.reportee_count / pm.max_capacity) >= 0.9).length || 0 },
  ];
          ...
          <table className="w-full">
            ...
            <tbody>
              {capacityReport?.slice(0, 10).map((pm: any, idx: number) => {
                const utilization = ((pm.reportee_count / pm.max_capacity) * 100).toFixed(0);
                return (
                  <tr key={idx} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{pm.max_capacity}</td>
