...
export default function GADAnalysis() {
  ...
  const { data: pmListData } = useGetPMsListQuery({ 
    page: 1, 
    pageSize: 1000,
  ...
  const { data: sameGradeData, isLoading: sameGradeLoading } = useGetSameGradeExceptionsQuery({ page, pageSize: PAGE_SIZE, practice: practiceParam, pmId: pmIdParam, ...departmentIdParam }, { skip: activeTab !== 'same_grade' });
  const { data: proposedData,  isLoading: proposedLoading  } = useGetProposedPMsQuery({ page, pageSize: PAGE_SIZE, practice: practiceParam, ...departmentIdParam }, { skip: activeTab !== 'proposed' });
  ...
  const dl = async (basePath: string, extra: Record<string, string> = {}) => {
    try {
      const url = new URL(basePath, window.location.origin);
      if (practiceParam) url.searchParams.set('practice', practiceParam);
      if (pmIdParam) url.searchParams.set('pm_id', pmIdParam);
      if (isSuperAdmin && selectedDepartment) url.searchParams.set('department_id', selectedDepartment.toString());
      ...
      a.href = downloadUrl;
      a.download = `${basePath.split('/').pop()}_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
      URL.revokeObjectURL(downloadUrl);
    } catch (error) {
      console.error('CSV download failed:', error);
      alert('CSV download failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 bg-white border rounded-xl px-4 py-3 shadow-sm">
        ...
        <div className="relative">
          ...
          >
            ...
            {practiceList.filter((p: string) => p !== 'All').map((p: string) => (
        ...
        <div className="p-5">
          ...
          {activeTab === 'correct' && (
            <div>
              ...
              {/* §1 GAD Classification Rule note */}
              <div className="mb-3 flex items-start gap-2 rounded-lg bg-blue-50 border border-blue-200 px-4 py-2 text-xs text-blue-700">
                <span className="mt-0.5">ℹ️</span>
                <span>
                  <strong>GAD Classification Rule:</strong> Employees marked <span className="font-semibold">No Better PM Available</span> have a structural mismatch (e.g. wrong region, grade, or BU) but no eligible replacement PM exists in their practice. Per the PM Assignment Spec §1, they are classified as <span className="font-semibold text-green-700">Correctly Mapped</span> — the current PM is the best available and must not be flagged as incorrect.
                </span>
              </div>
              <div className="overflow-x-auto rounded-lg border">
                <table className="w-full">
                  <thead><tr className="bg-gray-50 border-b">
                    ...
                    <Th>Sub-Practice</Th><Th>Region</Th><Th>PM Name</Th><Th>PM Grade</Th><Th>Status</Th>
                  </tr></thead>
                  <tbody>
                    {correctLoading ? (
                      ...
                      <tr key={r.employee_id} className={`border-b hover:bg-green-50/40 ${r.no_better_pm_available ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <Td>
                          {r.no_better_pm_available ? (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 border border-blue-200" title="hasSuggestedPM=FALSE — no eligible replacement PM in this practice. Correctly Mapped per spec §1.">
                              No Better PM
                            </span>
                          ) : (
