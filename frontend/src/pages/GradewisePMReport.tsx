...
const UtilizationBar = ({ pct }: { pct: number }) => (
  <div className="flex items-center gap-2">
    ...
    <span
      className={`text-xs px-1.5 py-0.5 rounded font-semibold ${utilizationTextColor(pct)}`}
    >
      {pct?.toFixed(0) ?? 0}%
    </span>
  </div>
...
export default function GradewisePMReport() {
  ...
  const locationOptions = [
    ...new Set(drilldown.map((r: any) => r.location).filter(Boolean)),
  ].sort() as string[];
  const skillOptions = [
    ...new Set(drilldown.map((r: any) => r.skill).filter(Boolean)),
  ].sort() as string[];
  ...
  const hasFilters = !!(search || filterLocation || filterSkill || filterUtil);
  ...
  const filteredDrilldown = drilldown.filter((pm: any) => {
    ...
    if (
      ...
      !pm.employee_id?.toLowerCase().includes(search.toLowerCase())
    )
      return false;
  ...
  const drillTotalPages = Math.max(
    1,
    Math.ceil(filteredDrilldown.length / DRILL_PAGE_SIZE),
  );
  const pagedDrilldown = filteredDrilldown.slice(
    (drillPage - 1) * DRILL_PAGE_SIZE,
    drillPage * DRILL_PAGE_SIZE,
  );

  const totalPMs = overview.reduce(
    (s: number, r: any) => s + (r.total_pms ?? 0),
    0,
  );
  const totalCapacity = overview.reduce(
    (s: number, r: any) => s + (r.total_capacity ?? 0),
    0,
  );
  const totalReportees = overview.reduce(
    (s: number, r: any) => s + (r.total_reportees ?? 0),
    0,
  );
  const overallUtilization =
    totalCapacity > 0 ? (totalReportees / totalCapacity) * 100 : 0;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              {selectedGrade
                ? `Grade ${selectedGrade} — PM Drill-down`
                : "Gradewise PM Capacity Report"}
            </h1>
      ...
      {!selectedGrade && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <p
              className={`text-3xl font-bold ${overallUtilization >= 90 ? "text-red-600" : overallUtilization >= 80 ? "text-orange-500" : "text-green-600"}`}
            >
          ...
          {overviewLoading ? (
            ...
            <div className="text-center py-16 text-gray-400">
              ...
              <p className="text-sm mt-1">
                Upload a GAD report to populate PM records
              </p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-6 py-3 text-left font-semibold text-gray-700 min-w-[160px]">
                    Utilization
                  </th>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
            ...
            <span className="ml-2 px-2.5 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-bold">
              ...
              {hasFilters && drilldown.length !== filteredDrilldown.length
                ? ` of ${drilldown.length}`
                : ""}{" "}
              PMs
            </span>
            ...
            <div className="text-center py-16 text-gray-400">
              ...
              <p className="font-medium">
                {hasFilters
                  ...
                  : `No active PMs at grade ${selectedGrade}`}
              </p>
              {hasFilters && (
                <button
                  onClick={clearFilters}
                  className="mt-2 text-sm text-[#0070AD] hover:underline"
                >
                  Clear filters
                </button>
              )}
