...
export default function Dashboard() {
  ...
  const { data: stats, isLoading: statsLoading } =
    useGetDashboardStatsQuery(dashboardParams);
  const { data: capacityReport, isLoading: capacityLoading } =
    useGetPMCapacityReportQuery(dashboardParams);
  ...
  const capacityData =
    capacityReport?.slice(0, 10).map((pm: any) => ({
  ...
  const utilizationData = [
    {
      name: "Under 50%",
      value:
        capacityReport?.filter(
          (pm: any) =>
            pm.reportee_count /
              (pm.spec_capacity_cap ?? pm.max_capacity ?? 10) <
            0.5,
        ).length || 0,
    },
    {
      name: "50-75%",
      value:
        capacityReport?.filter((pm: any) => {
          const util =
            pm.reportee_count / (pm.spec_capacity_cap ?? pm.max_capacity ?? 10);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-capgemini-gray-600 mt-1">
          Overview of PM alignment system
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Bar Chart */}
        <div className="card">
          <h2 className="text-lg font-semibold text-capgemini-gray-800 mb-4">
            Top 10 PM Capacity
          </h2>
          ...
          <table className="w-full">
            <thead>
              <tr
                ...
                style={{ borderBottom: "2px solid #12ABDB" }}
              >
