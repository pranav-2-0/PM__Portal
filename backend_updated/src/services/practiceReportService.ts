...
/**
 * Practice-Wise Report Generation Service
 * Generates comprehensive reports with business rules applied
 */
...
export interface PMCapacityMetrics {
  totalPMs: number;
  overCapacity: number;      // ≥90%
  highUtilization: number;   // 80-89%
  optimal: number;           // 50-79%
  underUtilized: number;     // <50%
  averageUtilization: number;
  overCapacityList: any[];
  underUtilizedList: any[];
}
...
export interface PracticeReportData {
  ...
  region: string;
  generatedAt: Date;
  pmCapacity: PMCapacityMetrics;
  bench: BenchMetrics;
  separations: SeparationMetrics;
  employees: {
    total: number;
    withPM: number;
    withoutPM: number;
...
export class PracticeReportService {
   ...
   */
  async generatePracticeReport(filters: PracticeReportFilters): Promise<PracticeReportData> {
   ...
   */
  private async getPMCapacityMetrics(whereClause: string): Promise<PMCapacityMetrics> {
    const query = `
      SELECT 
        pm.*,
        CASE 
   ...
   */
  private async getBenchMetrics(whereClause: string): Promise<BenchMetrics> {
    const query = `
      SELECT 
        e.*,
        CASE 
   ...
   */
  private async getSeparationMetrics(whereClause: string): Promise<SeparationMetrics> {
    const query = `
      SELECT 
        sr.*,
        pm.name as pm_name,
   ...
   */
  private async getEmployeeMetrics(whereClause: string): Promise<{
    total: number;
    withPM: number;
    withoutPM: number;
    newJoiners: number;
   ...
   */
  private buildWhereClause(filters: PracticeReportFilters): string {
   ...
   */
  async getPracticesList(): Promise<string[]> {
   ...
   */
  async getCUsList(): Promise<string[]> {
    const query = `
   ...
   */
  async getRegionsList(): Promise<string[]> {
    const query = `
...
export const practiceReportService = new PracticeReportService();
