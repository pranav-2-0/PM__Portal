-- People Manager Alignment System - Database Schema
-- PostgreSQL 15+
...
CREATE TABLE separation_reports (
    ...
    -- Person details extracted from the separation file
    person_name VARCHAR(200),
    grade VARCHAR(20),
    designation VARCHAR(200),
    -- Classification: 'pm' | 'employee' | 'unknown'
    person_type VARCHAR(20) DEFAULT 'unknown',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    -- NOTE: No FK to people_managers — separation file contains all employee types
);
...
CREATE TABLE pm_assignments (
    ...
    assignment_type VARCHAR(50) NOT NULL, -- 'new_joiner', 'reassignment'
...
CREATE TABLE approval_workflows (
    ...
    approver_role VARCHAR(50) NOT NULL, -- 'old_pm', 'new_pm', 'dcx'
...
-- Audit Trail (Phase 5: Enhanced audit tracking)
...
-- Configuration (for weights and rules)
...
('matching_weights', '{
    ...
    "grade": 3,
    "capacity": 2
}'::jsonb, 'Weights for PM matching algorithm (Updated: Skill increased from 5 to 20 as mandatory criterion)'),
('reportee_limits', '{
    "c1_c2": 10,
    "d1_d2_d3": 15,
    "capacity_warning_threshold": 80
}'::jsonb, 'Grade-based reportee limits and warning thresholds'),
('sla_hours', '{
    "new_joiner_assignment": 24,
    "reassignment": 48,
    "approval_reminder": 24,
    "approval_escalation": 48
}'::jsonb, 'SLA timings in hours'),
('notification_triggers', '{
    "lwd_t60": 60,
    "lwd_t30": 30,
    "lwd_t7": 7
}'::jsonb, 'Days before LWD for notifications');
